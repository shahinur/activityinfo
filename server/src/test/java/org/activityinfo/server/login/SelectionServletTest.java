...
public class SelectionServletTest {
    ...
    public void markManifestAsObsoleteIfAuthTokenIsBadOrExpired()
        ...
        // 9. User refreshes
        // 10. User agent receives 500 status code while trying to download new
        // manifest (because authToken is invalid)
        // so user agent does not know there is a new version
        // 11. Goto step 7, repeat infinitely.

        // To get the CORRECT behavior, the server needs to respond with status
        // code 404
        // to the request for the AppCache manifest, which will result in the
        // cache being
        // marked as obsolete and should force the user agent to delete the
        // cache, allowing
