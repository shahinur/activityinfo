...
import org.activityinfo.ui.style.tools.rebind.UserAgent;
...
public class GssTree {
    ...
    private void findClassNames() {
        ...
        RecordingSubstitutionMap recordingSubstitutionMap = new RecordingSubstitutionMap(new IdentitySubstitutionMap(),
    ...
    public void eliminatedDeadStyles(TreeLogger logger) {
        ...
        LoggingErrorManager errorManager = new LoggingErrorManager(logger.branch(Type.INFO, "Optimizing CSS Tree"));
    ...
    public void stripVendorPrefixed(TreeLogger logger, UserAgent userAgent) throws UnableToCompleteException {
        ...
        new RemoveVendorSpecificProperties(vendorOf(userAgent), cssTree.getMutatingVisitController()).runPass();
    }
    ...
    public void emitResources(TreeLogger logger,
                              SourceResolver resolver,
                              ResourceWriter writer) throws UnableToCompleteException {
        EmitResources emitter = new EmitResources(cssTree.getMutatingVisitController(), logger, resolver, writer);
        emitter.runPass();
