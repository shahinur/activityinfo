...
public class SiteUpdateBuilder implements UpdateBuilder {
    ...
    private void insertUpdatedIndicatorValues() {
        ...
        batch.insert().into(Tables.INDICATOR_VALUE).from(ivQuery).execute(entityManager);
    }

    private void insertUpdatedReportingPeriods() {
        SqlQuery rpQuery = updatedPeriodQuery();

        batch.insert().into(Tables.REPORTING_PERIOD).from(rpQuery).execute(entityManager);
    }

    private void insertUpdatedAttributeValues() {
        SqlQuery avQuery = updatedAttributeValuesQuery();

        batch.insert().into(Tables.ATTRIBUTE_VALUE).from(avQuery).execute(entityManager);
    }
    ...
    private void insertUpdatedSites() {
        ...
        // insert sites
        batch.insert().into(Tables.SITE).from(siteQuery).execute(entityManager);
    }
    ...
    private SqlQuery updatedIndicatorValueQuery() {
        return SqlQuery.select()
                       ...
                       .leftJoin(Tables.SITE, "s")
                       .on("rp.SiteId = s.SiteId")
                       .leftJoin(Tables.ACTIVITY, "a")
