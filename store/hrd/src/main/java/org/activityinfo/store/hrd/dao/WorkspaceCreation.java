...
public class WorkspaceCreation {
    ...
    public void createWorkspace(Resource resource) {
        createWorkspace(resource, Collections.<Resource>emptySet());
    }

    public void createWorkspace(Resource resource, Iterable<Resource> resourcesToCreate) {
        ...
        try(ReadWriteTx tx = ReadWriteTx.serializedCrossGroup()) {
            ...
            // Now write out any other initial resources that should be part of this tx
            for(Resource toCreate : resourcesToCreate) {
                workspaceUpdate.createResource(toCreate);
            }
